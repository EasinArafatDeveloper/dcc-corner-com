...
export function MiddlePromoBanner({ banner }: MiddlePromoBannerProps) {
  ...
  return (
    <section className="py-6 sm:py-10 bg-gradient-to-b from-transparent via-[#FAF7F2] to-transparent">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <Link 
          href={targetLink}
          className="group relative block w-full overflow-hidden rounded-2xl sm:rounded-3xl shadow-md border border-[#E8E0D5] bg-white transition-all duration-500 hover:shadow-2xl hover:border-[#C5A059]/50"
        >
          ...
          <div className="relative w-full aspect-[2.4/1] sm:aspect-[3.6/1] lg:aspect-[4.2/1] overflow-hidden bg-slate-900">
            ...
            <div className="absolute top-3 left-3 sm:top-4 sm:left-5 z-10">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#4A2C2A]/90 backdrop-blur-md text-[#C5A059] text-xs font-bold rounded-full shadow-lg border border-[#C5A059]/30">
                <Sparkles className="w-3.5 h-3.5 fill-[#C5A059]" /> Exclusive Offer
              </span>
            </div>

            {/* Floating Action Button indicator on desktop hover */}
            <div className="absolute bottom-3 right-3 sm:bottom-4 sm:right-5 z-10">
              <div className="inline-flex items-center gap-1.5 px-4 py-2 bg-white/90 backdrop-blur-md text-[#4A2C2A] text-xs sm:text-sm font-extrabold rounded-xl shadow-lg border border-[#E8E0D5] transform transition-all duration-300 group-hover:bg-[#4A2C2A] group-hover:text-white group-hover:scale-105">
                Explore Offer <ArrowRight className="w-3.5 h-3.5 text-[#C5A059]" />
              </div>
            </div>
          </div>
        </Link>
